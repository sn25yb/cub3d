...
void	put_playerimg(t_img2d *ret, t_img2d src, double rad)
{
	...
	while (xy.y < src.size.y)
	{
		...
		{
			nxy.x = cos(rad) * ((double)xy.x - cxy.x) + cxy.x - sin(rad) * ((double)xy.y - cxy.y) + 0.001;
			nxy.y = cos(rad) * ((double)xy.y - cxy.y) + sin(rad) * ((double)xy.x - cxy.x) + cxy.y + 0.001;
...
void	draw_player(t_game *game)
{
	...
	mlx_put_image_to_window(game->mlx, game->win, conv_img->image, 5 * 10 + 1, 5 * 10 + SCREEN_HEIGHT - 11 * 10);
	// mlx_put_image_to_window(game->mlx, game->win, player_img.image, 5 * 10 + 1, 5 * 10 + SCREEN_HEIGHT - 11 * 10);
}
